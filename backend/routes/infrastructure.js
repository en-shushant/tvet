...
async function plugin(fastify, opts) {
  ...
  fastify.post('/', { preHandler: requireWriter }, async (request, reply) => {
    const { institute_id, particular, description, unit, size, remark, sort_order } = request.body;
    if (!institute_id || !particular) return reply.code(400).send({ error: 'institute_id and particular required' });
    const { rows } = await pool.query(
      `INSERT INTO institute_infrastructure (institute_id, particular, description, unit, size, remark, sort_order)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [institute_id, particular, description || null, unit || null, size || null, remark || null, sort_order || 0]
    );
    return reply.code(201).send(rows[0]);
  });

  fastify.put('/:id', { preHandler: requireWriter }, async (request, reply) => {
    const { particular, description, unit, size, remark, sort_order } = request.body;
    if (!particular) return reply.code(400).send({ error: 'particular required' });
    const { rows } = await pool.query(
      `UPDATE institute_infrastructure SET particular=$1, description=$2, unit=$3, size=$4, remark=$5, sort_order=$6
       WHERE id=$7 RETURNING *`,
      [particular, description || null, unit || null, size || null, remark || null, sort_order || 0, request.params.id]
    );
