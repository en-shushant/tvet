...
async function plugin(fastify, opts) {
  ...
  fastify.post('/', { preHandler: requireWriter }, async (request, reply) => {
    const { client_id, institute_id, standing_list_name, fy,
            shortlist_date, valid_until, status, remarks } = request.body;
    ...
    const { rows: [row] } = await pool.query(
      `INSERT INTO shortlists
        (client_id, institute_id, standing_list_name, fy, shortlist_date, valid_until, status, remarks)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [client_id||null, institute_id, standing_list_name||null, fy||null, shortlist_date,
  ...
  fastify.put('/:id', { preHandler: requireWriter }, async (request, reply) => {
    ...
    const { rows } = await pool.query(
      `UPDATE shortlists SET client_id=$1, institute_id=$2, standing_list_name=$3,
        fy=$4, shortlist_date=$5, valid_until=$6, status=$7, remarks=$8,
        updated_at=NOW()
